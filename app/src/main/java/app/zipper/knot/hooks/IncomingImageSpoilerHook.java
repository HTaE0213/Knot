...
public class IncomingImageSpoilerHook implements BaseHook {
  ...
  private static final class CoveredImage {
    ...
    CoveredImage(ImageView image, float originalAlpha, Object originalRenderEffect) {
      this.image = image;
  ...
  private static void conceal(View root, String messageId, List<ImageView> candidates) {
    ...
    for (ImageView image : candidates) {
      ...
      Object originalEffect = null;

      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
        originalEffect = Api31.applyBlur(image, BLUR_RADIUS_DP * density);
      } else {
      ...
      state.images.add(new CoveredImage(image, originalAlpha, originalEffect));
  ...
  private static void clearSpoiler(View root) {
    ...
    if (value instanceof SpoilerState) {
      ...
      for (CoveredImage covered : state.images) {
        try {
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            Api31.restore(covered.image, covered.originalRenderEffect);
          } else {
  ...
  private static final class Api31 {
    ...
    static Object applyBlur(View view, float radiusPx) {
      RenderEffect original = view.getRenderEffect();
      view.setRenderEffect(
          RenderEffect.createBlurEffect(radiusPx, radiusPx, Shader.TileMode.CLAMP));
      return original;
    }

    static void restore(View view, Object effect) {
      view.setRenderEffect((RenderEffect) effect);
    }
  }
}
